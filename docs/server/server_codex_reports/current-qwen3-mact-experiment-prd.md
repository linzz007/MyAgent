...
| 当前进程状态 | 本轮 vLLM、`run_sharded_tqa.py`、`code/tqa.py` 均已关停；2026-07-30 21:44 复核无匹配模型/评测进程，`nvidia-smi` compute apps 为空 |
...
| P1 | 新模型筛选 | waiting: 2026-07-30 21:44 已递归复扫模型目录/缓存/挂载盘和外部 API env，仍无新增候选；GPU `4,5` 和 `6,7` 可用于下个候选的双服务 Gate；若审计 JSON 出现 `untested_local_model_paths`，直接取对应路径作为 `prepare_model_gate_run.py --model-id` |
