...
当前在线状态：2026-08-04 23:36 CST 两个 Qwen3-32B endpoint 按用户要求保持常驻：GPU `2,3` -> `http://127.0.0.1:8000/v1`，GPU `0,1` -> `http://127.0.0.1:8001/v1`，served model 均为 `qwen3-32b-local`。除非快速切换模型或用户要求释放，不主动停止服务。WTQ fresh、P4b after-targeted、E3 Seed-C/D current-only、E3 离线边界诊断、E3 budget probe、semantic-boundary plan、S2 after-guard fresh、S3 after-guard current-only rerun、Seed-D WTQ/TabFact boundary diagnosis/projection/fresh、S4 paired MACT、S5 CRT tie-breaker 和低成本细粒度机制消融审计均已完成；下一次在线实验不再优先刷 Qwen3 单数据集，而是等待新增模型/API 后进入 E4 gate。
