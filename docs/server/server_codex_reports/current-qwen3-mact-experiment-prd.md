...
| 当前阻塞条件 | `6,7` 仍有 GPU runtime 残留，但 `0,1,2,3` 已可运行 Qwen3-32B endpoint。2026-07-31 用户要求的 Qwen3-32B full200 目标已经按三数据集单项准确率过线；2026-08-03 P4b WTQ targeted fresh 和 after-targeted full50 已补齐，P4b 新 seed 单项也达到 WTQ/TabFact/CRT 三项均超过 MACT。剩余在线任务是 E3 Seed-C/Seed-D multi-seed current-only/paired，以及后续多模型 gate |
...
| 下一步建议 | 不建议把三个 coarse 变体都扩 full200。P4b after-targeted 已给出“WTQ/TabFact/CRT 三数据集单项均超过 MACT，overall/token 均过线”的新 seed 证据；下一步直接执行 E3 Seed-C/Seed-D current-only Gate-50，只有 current-only 达到 `decision=run_paired_mact` 才跑同 ID MACT paired，不直接扩全量 full200 |
