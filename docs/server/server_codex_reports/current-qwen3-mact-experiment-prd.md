...
快速 checkpoint（2026-08-04 23:03 CST）：S5 CRT tie-breaker 已完成并汇总。新增 gold-free CRT scalar canonicalization：负数 `difference` 答案转非负差值、country/nation 问题中的国家代码展开为国家名。重放旧 CRT100 从 `62/100` 投影到 `64/100`；affected-slice fresh `25` 条实跑为新 MyAgent `16/25` vs 旧 MyAgent `12/25` vs MACT `12/25`，failed/missing `0/0`；完整 CRT100 当前代码 fresh 为 MyAgent `65/100` vs MACT `62/100`，token ratio `0.7979`，failed/missing `0/0`。最终 S5 合并：WTQ `76/100 > 74/100`，TabFact `91/100 > 87/100`，CRT `65/100 > 62/100`，overall `232/300 > 223/300`，overall token ratio `0.5662`，overall failed/missing 为 MyAgent `0/0`、MACT `4/4`，decision=`s5_strict_all_dataset_pass`。用户定义的当前 Qwen3 strong patent strict 目标已达成；下一步转为正式实验包更新、机制消融补齐与新增模型/API gate，而不是继续刷 TabFact/CRT。

快速 checkpoint（2026-08-04 23:36 CST）：新增低成本细粒度机制消融审计，未启动模型，只读取冻结 artifact。结果目录：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_fine_grained_mechanism_ablation_audit_20260804_2333/`。decision=`fine_grained_mechanism_evidence_ready_for_qwen3_patent_scope_with_evidence_retention_boundary_and_e4_pending`。关键证据：`no_strong_verification` diagnostic Gate-50 相对 current `-8/150` 且 WTQ `-7/50`；`no_deterministic_shortcuts` `-15/150`，TabFact `-9/50`、CRT `-7/50`；S2 fresh guard baseline -> after-guard 从 representative recovered `4/12` 提升到 `8/12`、no-harm `17/18` 到 `18/18`、weighted token ratio `0.6975 -> 0.6104`；S5 CRT scalar canonicalization replay 为 `64/100 > 62/100`，full CRT100 fresh 为 `65/100 > 62/100`。R2 机制消融当前可写；边界是 evidence-retention 仍没有单独 no-evidence-retention 因果开关，E4 仍无新增模型/API 候选。

快速 checkpoint（2026-08-04 23:52 CST）：复核并刷新 E4 multi-model readiness，未启动新模型长跑。最新 artifact：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/e4_multimodel_gate_readiness_audit_20260804_235201_zh.md`，latest 入口仍为 `latest_e4_multimodel_gate_readiness_audit_zh.md`。decision=`no_candidate_wait`，can_start_gate10_now=`False`，local models discovered=`4`，untested local models=`0`，API keys/provider profiles=`0/0`。当前只有两个可见模型进程，均为用户要求常驻的 Qwen3-32B endpoint：GPU `2,3` -> `8000`，GPU `0,1` -> `8001`；没有评测 runner 残留。结论：E4 继续等待新增本地模型或 API key，不重跑已 no-go 模型，不释放 Qwen3 显存。

快速 checkpoint（2026-08-05 00:02 CST）：日期切换/疑似扩容后再次复核 E4 multi-model readiness，未启动新模型长跑。最新 artifact：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/e4_multimodel_gate_readiness_audit_20260805_000234_zh.md`，latest 入口仍为 `latest_e4_multimodel_gate_readiness_audit_zh.md`。decision=`no_candidate_wait`，can_start_gate10_now=`False`，local models discovered=`4`，untested local models=`0`，API keys/provider profiles=`0/0`。当前只有两个可见模型进程，均为用户要求常驻的 Qwen3-32B endpoint：GPU `2,3` -> `8000`，GPU `0,1` -> `8001`；没有评测 runner 残留。结论不变：E4 继续等待新增本地模型或 API key，不重跑已 no-go 模型，不释放 Qwen3 显存。
...
| 当前正在执行 | 当前 Qwen3 strong patent strict 主目标已完成：S5 合并结果为 MyAgent `232/300` vs MACT `223/300`，WTQ/TabFact/CRT 三项均严格超过 MACT，overall token ratio `0.5662`，overall failed/missing 为 MyAgent `0/0`、MACT `4/4`。S5 已写入正式实验包，细粒度机制消融审计已补到可写状态；2026-08-05 00:02 已刷新 E4 readiness，仍无新增模型/API 候选。下一步不再优先优化 TabFact/CRT 单点，只等待新增模型或外部 API key 后按 Gate-10 -> Gate-50 -> Gate-150 -> paired-200 漏斗扩展，或在专利代理人要求时补单独 no-evidence-retention 开关 |
| 当前最新增量 | 2026-08-04 22:25-2026-08-05 00:02：创建 S5 CRT tie-breaker 目录 `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_s5_crt_tiebreaker_diag_20260804_2225/` 并完成 strict pass；随后创建细粒度机制消融审计目录 `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_fine_grained_mechanism_ablation_audit_20260804_2333/`，用冻结 coarse ablation、S2 before/after fresh、S5 replay/fresh 证据收口 R2 机制证据；00:02 再刷新 E4 readiness 为 `no_candidate_wait`。MyAgent 代码只改 `code/my_agents.py` 的 CRT scalar canonicalizer，测试只补 `tests/test_myagent_pipeline.py` 的 CRT difference/country-code 用例 |
| 当前最新结论（覆盖旧 S4 strict fail 状态） | 2026-08-04 23:03：S4 的 CRT 持平问题已通过 S5 CRT full100 当前代码 fresh 闭环。最终同口径 Seed-C/D Gate-50 paired 结果：WTQ MyAgent `76/100` vs MACT `74/100`，TabFact `91/100` vs `87/100`，CRT `65/100` vs `62/100`，overall `232/300` vs `223/300`，overall token ratio `0.5662`，overall failed/missing 为 MyAgent `0/0`、MACT `4/4`。可以写成“Qwen3-32B + MyAgent 在当前 paired 多 seed 三数据集上均超过 MACT，且总体 token 明显低于 MACT”。边界：CRT 单项 token ratio `0.7979`，仍低于 MACT 但不低于 `0.75`；多模型稳定性仍 pending |
| 当前本轮代码优化 | WTQ：答案形态/失败状态驱动的 high-confidence verifier 劝返门控、existing-total-row shortcut、planner `Ellipsis` 占位符执行拦截、晚列证据行召回、earlier/later 候选比较保留全局行、否定年份标量冲突的高置信审阅者劝返；2026-08-01 E2 targeted fixes 新增 listed-after 目标列、directly-before 目标列、overtime `(OT)` 标记计数、rank/country suffix person canonicalization、playoff parenthetical negator、division winner entry count、unique sponsor count、retired-injured ordinal attempt；2026-08-04 v6c 新增 route-after-stop、same-number entity、year-span duration、consecutive-month count、score-pair low-score entity、year-header value、rank gap、explicit option absence、metric-value entity list、strict threshold count、inferred rank 等 gold-free WTQ 边界 shortcut。TabFact：国家配对、零金牌计数、日期前全胜、venue/competition/date 同行匹配、score-but-lose、second-smallest metric、retirement threshold、v6b 实体属性/同行/计数/双实体/时间差/数值差审计；2026-08-04 v6c 新增条件指标值、实体总分比较、replay 月份计数、最低 attendance 周次、replay home-team draw/win 核验、tenure contains 与 aircraft call-sign strict row guard。CRT S5：新增 gold-free scalar canonicalization，覆盖负数 `difference` 答案非负化与 country/nation 问题国家代码展开 |
...
| 当前进程状态 | 2026-08-03 12:45：已把 runtime preflight 写入 MACT 包：`qwen3_runtime_preflight_20260803_124508_zh.md`，状态为 `blocked_gpu_runtime_residual`；`8000/8001` endpoint 不健康，`0-3` 约 `27.8GB/卡` 且 util 在 `68%-100%`，compute-app PID 为空。E4 readiness 同步刷新为 `e4_multimodel_gate_readiness_audit_20260803_124508_zh.md`，仍为 `no_candidate_wait`，不能启动 Gate-10 |
...
| 当前进程状态 | 2026-08-04 17:55：S4 paired MACT 已完成 seed_c/WTQ `50/50` 和 seed_c/TabFact `50/50`；seed_c/CRT 正在 `8001` 跑，输出 `4/50`，pid `185751/185759`；seed_d/WTQ 正在 `8000` 跑，输出 `1/50`，pid `187054/187060`。seed_c/WTQ 的 runner session 最后返回过 `line 62: 3: command not found`，但文件已完整 50 行；当前脚本 `bash -n` 通过，后续用同一脚本 `--resume` 继续。若会话中断，先 `wc -l` 检查 S4 目录，再补跑未满 50 行的数据集 |
| 当前进程状态 | 2026-08-04 18:52：S4 paired MACT 继续运行；seed_c/CRT 正在 `8001` 跑，输出 `23/50`，seed_d/WTQ 正在 `8000` 跑，输出 `26/50`。seed_d/WTQ 当前有 1 条 MACT exec_error（`nu-3573`）。下一步保持这两路，seed_d/WTQ 完成后优先在 `8000` 启动 seed_d/CRT，让两个 CRT 并行；TabFact 放到后续空闲端口 |
...
| 最新细粒度机制消融审计 | `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_fine_grained_mechanism_ablation_audit_20260804_2333/summary/fine_grained_mechanism_ablation_audit.md`；将 coarse `no_strong_verification`/`no_deterministic_shortcuts`、S2 guard baseline-vs-after、S5 CRT scalar canonicalization replay/fresh 汇总为专利可写的机制证据。结论：risk collaboration / persuasion-back、deterministic audit、CRT scalar canonicalization、budget control 均可写；evidence-retention 可作为 supporting evidence，若要窄因果拆分再补 no-evidence-retention |
| 最新 multi-seed 执行包 | 旧 E3 包：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_multiseed_gate50_20260801_2231/`；原始 Seed-C/D current-only 合计 `212/300`，作为历史边界基线。S3/S4/S5 递进已完成：S3 after-guard current-only、Seed-D boundary fresh、S4 paired MACT、S5 CRT tie-breaker full100。最新严格通过目录：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_s5_crt_tiebreaker_diag_20260804_2225/summary/e3_s5_final_combined_summary.md`，结论为 WTQ `76/100 > 74/100`、TabFact `91/100 > 87/100`、CRT `65/100 > 62/100`、overall `232/300 > 223/300`、token ratio `0.5662`、overall failed/missing MyAgent `0/0`、MACT `4/4` |
...
| 最新 E4 多模型 readiness | `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/latest_e4_multimodel_gate_readiness_audit_zh.md`；2026-08-05 00:02 已刷新，timestamped 证据为 `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/e4_multimodel_gate_readiness_audit_20260805_000234_zh.md`。decision=`no_candidate_wait`，can_start_gate10_now=`False`，local models discovered=`4`，untested local models=`0`，API keys/provider profiles=`0/0`，visible model/runner processes=`2`，default GPU pool `0-3` available=`false`，原因是两个 Qwen3-32B endpoint 按用户要求常驻占用 `0,1,2,3`，且没有新增模型/API 候选 |
...
| 最新当前专利实验章节 | `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/latest_current_patent_experiment_section_zh.md`；2026-08-04 23:03 已刷新，当前从 full200、P4b after-targeted、机制矩阵、E3 边界诊断、E3 budget probe、E3 semantic-boundary plan、E3 S2/S3/v6c、S4 paired MACT、S5 CRT tie-breaker 和 E4 readiness 汇总，结论为 `s5_strict_all_dataset_pass`。它是给专家/专利代理人看的实验章节收口稿，可以写当前 Qwen3 paired 多 seed 三数据集均超过 MACT，但不能写多模型已验证 |
| 最新完成度审计 | `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/latest_completion_gap_audit_current_zh.md`；2026-08-04 23:36 已补细粒度机制审计索引，逐项确认 R1 full200 主证据完成、R2 机制证据已达到当前 Qwen3 专利撰写可用状态、R3 WTQ fresh/after-targeted 闭环完成、R4 已推进到 `complete_strict_all_dataset_pass`：S5 final MyAgent `232/300` vs MACT `223/300`、token ratio `0.5662`、overall failed/missing MyAgent `0/0`、MACT `4/4`。剩余硬缺口是 E4 新模型/API 候选；可选缺口是专利代理人要求时再补 standalone no-evidence-retention |
| 最新权利要求证据矩阵 | `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/claim_evidence_traceability_20260801_2248_zh.md`；2026-08-04 23:03 已刷新，把 WTQ targeted fresh/P4b after-targeted、E3 S2/S3/v6c、S4 paired 和 S5 strict pass 串成完整证据链；C4/C6 的 Qwen3 strong strict 缺口已闭合，剩余为 E4 外部模型证据和可选细粒度因果消融 |
| 最新专利说明书初稿 | `/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/patent_disclosure_draft_zh.md`；2026-08-04 23:03 已补“实施例十：E3 S5 CRT tie-breaker 与 paired strict pass”，包括 CRT100 fresh `65/100 > 62/100`、S5 combined `232/300 > 223/300`、token ratio `0.5662`、overall failed/missing MyAgent `0/0`、MACT `4/4`，并把多模型 readiness 顺延为实施例十一 |
| 最新正式结果表/模板 | 当前台账：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/latest_formal_result_ledger_current_zh.md`；模板：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/formal_result_tables_template_20260801_2252_zh.md`。2026-08-04 23:03 台账已包含 WTQ fresh、P4b after-targeted、E3 current-only、S3 after-guard、E3 v6c boundary-fresh candidate、S4 paired MACT、S5 CRT tie-breaker 和 E4 no-candidate；completed rows `48`，pending rows `1` |
| 下次本地模型服务资源 | 2026-08-05 00:02 最新口径：当前已有两个 Qwen3-32B endpoint 常驻，GPU `2,3` -> `http://127.0.0.1:8000/v1`，GPU `0,1` -> `http://127.0.0.1:8001/v1`；两个模型名均为 `qwen3-32b-local`，API key env 为 `LOCAL_VLLM_API_KEY=local-vllm-key-change-me`。后续 MyAgent 小规模 sanity/gate 可用 `VLLM_ENDPOINTS=http://127.0.0.1:8000/v1,http://127.0.0.1:8001/v1` 双端点并行。`4,5,6,7` 仍有约 `42GB/卡` 占用，但当前没有新增模型/API 候选，因此不启动 E4 Gate-10 |
| 用户最新资源补充 | 2026-08-04 10:49 用户提示 `0,1,2,3` 卡释放并要求启动后不要释放显存；已遵守为双实例常驻：`2,3 -> 8000` 与 `0,1 -> 8001`。除非快速切换模型或用户明确要求释放，不关闭这两个 Qwen3 服务 |
| 当前本机模型候选 | 2026-08-05 00:02 E4 审计 `/home/ubuntu/models`、`/home/ubuntu/.cache/huggingface`、`/data`、`/mnt` 后只发现 Qwen3-32B、Qwen3-14B-AWQ、Qwen2.5-14B-Instruct-AWQ、Qwen2.5-3B-Instruct；这些都属于已测/已 no-go 清单，`untested_local_models=[]`、`untested_local_model_paths={}`；当前不能生成新的本地模型 Gate run |
| 当前外部 API 候选 | 2026-08-05 00:02 E4 审计显示环境变量和现有真实 `configs/server/*.env` 均未发现可用 API key，`api_keys_present=[]`、`api_provider_profiles={}`；外部 API Gate 仍需用户提供 key 和目标 provider model 后才能生成 run |
| 当前阻塞条件 | Qwen3-32B strong patent strict 主目标不再阻塞：S5 已证明三数据集 paired 多 seed 均严格超过 MACT，且总体 token 明显更低。剩余外部阻塞只剩 E4：没有新增本地模型或外部 API key，latest E4 readiness 为 `no_candidate_wait`；因此当前不能确认“多模型稳定”，但可以把 Qwen3 当前主线写成严格达标阶段结论 |
...
| 下一步建议 | 不建议继续为了单点分数刷 TabFact/CRT。S5 strict pass、正式实验包同步和低成本细粒度机制消融审计都已完成；下一步只在出现新增本地模型或 API key 后按 Gate-10 -> Gate-50 -> Gate-150 -> paired-200 漏斗做多模型外延。若专利代理人要求更窄因果证据，再补 standalone no-evidence-retention 或 no-WTQ-verifier-override 小样本开关。Qwen3 服务按用户要求保持常驻，不主动释放显存 |
...
### 0.2 2026-08-04 E3 边界预算 probe 台账

本轮目标：不继续优化已经通过的 Qwen3 full200/P4b after-targeted 主锚点，而是针对 E3 Seed-C/D 的稳定性边界做低成本机制验证，判断代表错题是否只是 `max_replan=3` 预算不足。

执行与结果：

1. 复核 `8000` Qwen3-32B 服务健康后，构建 E3 representative wrong rows probe。源诊断 Markdown 实际保存每个 seed/dataset 两条代表错题，因此可审计样本数为 `12` 条，不使用口头记录里的 `18` 条。
2. 运行 `max_replan=5` MyAgent probe，输出到 MACT：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_boundary_budget_probe_20260804_1035/`。
3. 结果：原始 `max_replan=3` 代表错题 `0/12` 正确；`max_replan=5` 后 `4/12` 正确，failed/missing `0/0`，avg tokens `12444.9 -> 13136.1`，avg elapsed `45.21s`。
4. 分项：WTQ `1/4` recovered，TabFact `3/4` recovered，CRT `0/4` recovered。TabFact temporal/numeric 和部分 WTQ temporal 对预算敏感；WTQ entity lookup/row selection 与 CRT numeric/span 类代表错题没有被 blanket replan budget 修复。
5. 判定：`mixed_budget_sensitivity_not_enough_for_e3_stability`。这可以写成 adaptive budget 机制证据和 remaining semantic-boundary 证据，不能写成 E3 稳定性闭环。
6. 同步更新：MACT package latest runtime preflight、E4 readiness、current patent section、completion gap audit、专利说明书初稿、权利要求证据矩阵已刷新；两个 Qwen3 endpoint 常驻，`0,1 -> 8001` 与 `2,3 -> 8000`，后续除非用户要求或切模型，不释放显存。

关键文件：

```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_boundary_budget_probe_20260804_1035/README.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_boundary_budget_probe_20260804_1035/input/
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_boundary_budget_probe_20260804_1035/myagent_max_replan5/
...
### 0.3 2026-08-04 E3 语义边界计划台账

本轮目标：把 E3 budget probe 的结论转化成下一轮可执行、可写专利的实验漏斗，避免直接重跑 full200 或无条件扩大 replan 预算。

执行与结果：

1. 新建 MACT 计划目录：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_semantic_boundary_plan_20260804_1110/`。
2. `build_semantic_boundary_plan.py` 读取 E3 Seed-C/D 边界诊断和 E3 max_replan=5 probe，输出 `summary/e3_semantic_boundary_plan.json/md`。
3. 当前 decision：`do_not_rerun_full200_or_paired_mact_until_targeted_guards_pass`。
4. P0 零恢复类别：`crt_multi_step_numeric_composition_boundary`、`wtq_entity_lookup_or_row_selection_boundary`、`crt_span_or_universal_quantifier_boundary`、`tabfact_false_negative_entailment_boundary`。
5. high-priority work items：`8` 个，覆盖 P0/P1 semantic guard 与 mixed budget/semantic 类别。
6. 漏斗状态：S1 gold-free guard 设计/单测已完成；S2 12 条代表错题加 no-harm affected-slice fresh 已通过；S3 rerun E3 current-only 已完成但只通过 Seed-C，Seed-D WTQ/TabFact 未过；S4 paired MACT 未触发。
7. 已同步：MACT package `evidence_manifest.json`、`latest_current_patent_experiment_section_zh.md`、`latest_completion_gap_audit_current_zh.md`、`patent_disclosure_draft_zh.md`、`claim_evidence_traceability_20260801_2248_zh.md/json`，以及本 PRD。S2 fresh 结果见 0.5。

关键文件：

```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_semantic_boundary_plan_20260804_1110/README.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_semantic_boundary_plan_20260804_1110/build_semantic_boundary_plan.py
...
### 0.4 2026-08-04 E3 S2 guard-validation input package 台账

本轮目标：按照 0.3 的 semantic-boundary plan，先把后续 P0/P1 semantic guard 的小规模验证输入固定下来，避免直接花长时间重跑 full200、Seed-C/D 或 paired MACT。

执行与结果：

1. 新建 MACT 输入包目录：`/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/`。
2. 先写 `tests/test_build_guard_validation_inputs.py` 锁定输出规则；红灯为生成器缺失，补 `build_guard_validation_inputs.py` 后测试通过。
3. 生成输入包：总计 `30` 行，其中 `representative_wrong=12`，`no_harm_correct=18`；WTQ/TabFact/CRT 行数分别为 `10/8/12`。
4. 代表错题全部来自 E3 max_replan=5 budget probe 的 `12` 行；no-harm 行从 Seed-C/D 已正确样本中按同一边界分类器抽取，TabFact false-negative entailment 的 no-harm 控制使用 same-row/binary-entailment proxy，并在 metadata 中记录 `no_harm_proxy_for`。
5. 注册的 S2 fresh gate：实现 P0/P1 gold-free semantic guard 后，至少恢复 `7/12` 代表错题，同时保持 `18/18` no-harm 行正确，failed/missing 必须为 `0`，token ratio 不应超过 MACT full200 reference。
6. 当前状态：S2 after-guard fresh 已在 0.5 完成并通过，actual 为 `8/12` representative recovered、`18/18` no-harm、failed/missing `0/0`、weighted token ratio `0.6104`。该输入包本身仍是预注册输入，不是多 seed 稳定性正证据。

关键文件：

```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/README.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/build_guard_validation_inputs.py
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/tests/test_build_guard_validation_inputs.py
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/input/input_manifest.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/input/wtq_e3_guard_validation.jsonl
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/input/tabfact_e3_guard_validation.jsonl
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_inputs_20260804_1128/input/crt_e3_guard_validation.jsonl
...
### 0.5 2026-08-04 E3 S2 guard-validation fresh 台账

本轮目标：按 0.3/0.4 的漏斗，在不重跑 full200/paired MACT 的前提下，先验证 P0/P1 gold-free semantic guard 是否能修复 E3 Seed-C/D 边界错题，并确认不会伤害同类已正确样本。

已完成代码优化：

1. WTQ：新增多列条件目标列定位，覆盖 “at which age was overall=48 and giant slalom=48” 这类列值组合查找。
2. TabFact：新增编号列同队关系核验，覆盖 “X and Y play on the same team as no A and no B” 这类多实体同行/同编号列关系。
3. CRT：新增 numeric outlier yes/no 审计、top-k 年限平均、constructor 退休原因百分比三类 deterministic guard。
4. CRT 契约修复：显式 `Answer with only 'Yes' or 'No'` 优先于 `number ... compared to` 的 more/less/equal 启发式，避免 `Yes` 被 fallback 覆盖成 `more`。
5. CRT 输出规范化：整数百分比字符串从 `80.0%` 规范为 `80%`，保持旧评估口径下 no-harm 正确。

验证结果：

| run | rows | representative recovered | no-harm correct | failed/missing | weighted token ratio | decision |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| current baseline | 30/30 | 4/12 | 17/18 | 0/0 | 0.6975 | `baseline_needs_guard_implementation` |
| after-guard fresh | 30/30 | 8/12 | 18/18 | 0/0 | 0.6104 | `after_guard_passes_s2_gate` |

after-guard 分数据集结果：

| dataset | rows/eval/merged | representative recovered | no-harm correct | failed/missing | token ratio | avg tokens | avg seconds |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
仍未恢复的代表错题：WTQ `nu-2577`、WTQ `nu-3380`、CRT `crt-325`、CRT `crt-303`。这些保留为 S3 rerun 后的诊断候选，不在 S2 继续追求全清零，避免过拟合 affected slice。

关键文件：

```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_current_baseline_20260804_1142/summary/e3_guard_validation_current_baseline_summary.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_after_guard_20260804_1203/README.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_after_guard_20260804_1203/run_after_guard.sh
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_after_guard_20260804_1203/summarize_after_guard.py
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_after_guard_20260804_1203/myagent_after_guard/merged/wtq_qwen3-32b-local.jsonl
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_after_guard_20260804_1203/myagent_after_guard/merged/tabfact_qwen3-32b-local.jsonl
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_guard_validation_after_guard_20260804_1203/myagent_after_guard/merged/crt_qwen3-32b-local.jsonl
...
### 0.6 2026-08-04 E3 S3 after-guard current-only rerun 台账

本轮目标：在 S2 affected-slice/no-harm gate 通过后，用当前 MyAgent after-guard 代码重跑 E3 Seed-C/D current-only Gate-50，判断小样本 semantic guards 是否迁移到完整 `300` 行多 seed current-only。该阶段仍不跑 paired MACT；只有 Seed-C 和 Seed-D 都过当前 gate 才进入 S4。

执行结果：

| seed | dataset | rows input/merged/eval | correct | threshold | token ratio | avg tokens | avg seconds | failed/missing | gate |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| Seed-C | WTQ | 50/50/50 | 40/50 | 35/50 | 0.5885 | 6183.56 | 14.63 | 0/0 | pass |
| Seed-C | TabFact | 50/50/50 | 46/50 | 45/50 | 0.2556 | 2768.34 | 10.92 | 0/0 | pass |
| Seed-C | CRT | 50/50/50 | 32/50 | 30/50 | 0.9200 | 11785.16 | 25.72 | 0/0 | pass |
| Seed-C | aggregate | 150/150/150 | 118/150 | all dataset gates | 0.6073 | 6912.35 | 17.09 | 0/0 | `s3_seed_pass_run_paired_mact_candidate` |
| Seed-D | WTQ | 50/50/50 | 28/50 | 35/50 | 0.6417 | 6743.50 | 17.74 | 0/0 | inspect |
| Seed-D | TabFact | 50/50/50 | 39/50 | 45/50 | 0.2613 | 2829.94 | 11.32 | 0/0 | inspect |
| Seed-D | CRT | 50/50/50 | 30/50 | 30/50 | 0.7612 | 9751.56 | 25.23 | 0/0 | pass |
| Seed-D | aggregate | 150/150/150 | 97/150 | all dataset gates | 0.5659 | 6441.67 | 18.10 | 0/0 | `s3_seed_stop_or_inspect` |
| Combined | aggregate | 300/300/300 | 215/300 | both seeds pass | 0.5866 | 6677.01 | 17.59 | 0/0 | `s3_stop_or_inspect_boundary_remains` |

关键判断：

1. Seed-C 已通过 S3 current-only gate，说明 S2 semantic guards 至少在一组新 seed 上迁移有效。
2. Seed-D 未通过 S3：WTQ `28/50 < 35/50`、TabFact `39/50 < 45/50`；CRT 正好过门槛 `30/50`。
3. combined token ratio 仍明显低于 MACT full200 reference：`0.5866`；失败/缺答案为 `0/0`，因此阻塞不是 runner、工具、缺答案或 token 超预算。
4. paired MACT next 为 `false`，当前不能启动 S4 paired MACT，也不能写作“多 seed 稳定超过 MACT”。
5. 下一步若继续 Qwen3 内部优化，应先诊断 Seed-D WTQ/TabFact wrong rows，构造 affected-slice/no-harm 小包做 fresh；不要直接扩大 full200 或 paired MACT。

关键文件：

```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/README.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/run_s3_current.sh
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summarize_s3_current.py
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summary/seed_c_s3_current_summary.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summary/seed_d_s3_current_summary.json
...
### 0.7 2026-08-04 E3 Seed-D v6c boundary fresh 台账

本轮目标：承接 0.6 的 Seed-D WTQ/TabFact 边界失败，不直接重跑 full200 或 paired MACT，而是先做可解释、gold-free 的边界诊断和 deterministic semantic shortcut 投影；投影达到门槛且 harm 为 `0` 后，再只 fresh rerun Seed-D WTQ/TabFact，并继承未改代码路径的 Seed-D CRT 与 Seed-C S3 结果，判断是否进入 S4 paired MACT 候选。

诊断与投影：

| item | result |
|---|---|
| Seed-D WTQ+TabFact S3 current | `67/100`，current gate 需要 `80/100`，缺口 `13` |
| WTQ S3 current | `28/50`，threshold `35/50`，缺口 `7`；transitions 为 stable_right `27`、recovered `1`、regressed `3`、stable_wrong `19` |
| TabFact S3 current | `39/50`，threshold `45/50`，缺口 `6`；transitions 为 stable_right `36`、recovered `3`、regressed `2`、stable_wrong `9` |
| v6c boundary shortcut projection | WTQ+TabFact `67 -> 81/100`，threshold `80/100`，harm `0`，decision=`run_seed_d_wtq_tabfact_fresh_rerun` |
| projection 分项 | WTQ `28 -> 36/50`、TabFact `39 -> 45/50`，两项均达到 current gate |

fresh rerun 结果：

| dataset | evidence | input/merged/eval | correct | threshold | token ratio | avg tokens | avg seconds | failed/missing | gate |
|---|---|---:|---:|---:|---:|---:|---:|---:|---|
| WTQ | fresh_v6c_boundary_rerun | 50/50/50 | 36/50 | 35/50 | 0.6217 | 6533.2 | 15.75 | 0/0 | pass |
| TabFact | fresh_v6c_boundary_rerun | 50/50/50 | 45/50 | 45/50 | 0.2356 | 2552.2 | 10.25 | 0/0 | pass |
| CRT | inherited_from_s3_no_code_path_change | 50/50/50 | 30/50 | 30/50 | 0.7612 | 9751.6 | 25.23 | 0/0 | pass |
| Seed-D aggregate | mixed fresh/inherited | 150/150/150 | 111/150 | all dataset gates | 0.5516 | - | - | 0/0 | `seed_d_boundary_fresh_passes_current_gate` |

combined current-only candidate：

| seed | evidence | correct | token ratio | failed/missing | decision |
|---|---|---:|---:|---:|---|
| Seed-C | inherited_seed_c_s3 | 118/150 | 0.6073 | 0/0 | `s3_seed_pass_run_paired_mact_candidate` |
| Seed-D | mixed fresh/inherited | 111/150 | 0.5516 | 0/0 | `seed_d_boundary_fresh_passes_current_gate` |
| Combined | mixed | 229/300 | 0.5794 | 0/0 | `boundary_fresh_pass_run_paired_mact_candidate` |

结论与限制：

1. 0.6 的 Seed-D WTQ/TabFact gate failure 当前已闭环为 v6c current-only pass；阻塞点不再是 Seed-D WTQ/TabFact。
2. WTQ 的 fresh 正确数采用 primary denotation metric，为 `36/50`；同一 eval 中 exact match 为 `34/50`，写实验表时必须注明口径。
3. 当前证据还不是 paired MACT：Seed-C 继承 S3，Seed-D CRT 继承 S3，只有 Seed-D WTQ/TabFact 是 v6c fresh。
4. 下一步可进入 S4 paired MACT；若专家要求所有 current-only 输入都同一代码版本 fresh，可先补完整 v6c S3 current-only rerun。
5. E4 多模型仍等待新增本地模型或 API key，不能把当前 Qwen3 证据外推成多模型稳定性。

关键文件：

```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summary/seed_d_wtq_tabfact_after_guard_boundary_diagnosis.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summary/seed_d_wtq_tabfact_after_guard_boundary_diagnosis.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summary/seed_d_wtq_tabfact_boundary_shortcut_projection.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6b_e3_s3_current_rerun_after_guard_20260804_1425/summary/seed_d_wtq_tabfact_boundary_shortcut_projection.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/README.md
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/myagent_seed_d_boundary_fresh/merged/wtq_qwen3-32b-local.jsonl
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/myagent_seed_d_boundary_fresh/merged/tabfact_qwen3-32b-local.jsonl
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/myagent_seed_d_boundary_fresh/eval/wtq_qwen3-32b-local_eval.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/myagent_seed_d_boundary_fresh/eval/tabfact_qwen3-32b-local_eval.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/summary/seed_d_boundary_fresh_summary.json
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_seed_d_boundary_fresh_20260804_1549/summary/seed_d_boundary_fresh_summary.md
...
|---|---:|---:|---:|---|
| seed_c | WTQ | 50/50 | 3 | completed; old MACT 2048 max-token context-overflow/empty-row failures included |
| seed_c | TabFact | 50/50 | 0 | completed |
| seed_c | CRT | 4/50 | 0 | running on `http://127.0.0.1:8001/v1` |
| seed_d | WTQ | 1/50 | 0 | running on `http://127.0.0.1:8000/v1` |
...
| seed | dataset | output rows | exec_error rows | status |
|---|---:|---:|---:|---|
| seed_c | WTQ | 50/50 | 3 | completed |
| seed_c | TabFact | 50/50 | 0 | completed |
| seed_c | CRT | 23/50 | 0 | running on `http://127.0.0.1:8001/v1` |
...
20:08 时点结论边界：四组 MACT baseline 已完整落盘，两组仍在运行；S4 当时仍处于 `current_only_candidate_paired_pending`，最终结论必须等 `summarize_s4_paired.py` 生成 `summary/e3_s4_paired_combined_summary.json/md` 后再写。该历史等待状态已由后续 S4/S5 闭合。
...
21:22 时点结论边界：五组 MACT baseline 已完整落盘，只剩 `seed_d/CRT`。S4 当时仍处于 `current_only_candidate_paired_pending`，最终结论必须等 `summarize_s4_paired.py` 生成 `summary/e3_s4_paired_combined_summary.json/md` 后再写。该历史等待状态已由后续 S4/S5 闭合。
...
S4 时点结论边界：S4 可以写成“Qwen3-32B + MyAgent 在 paired Gate-50 多 seed 汇总上总体超过 MACT，且 token 明显更低；WTQ/TabFact 严格超过 MACT”。S4 不能写成“全部数据集严格超过 MACT”，因为 CRT combined 只是持平；该边界已由 0.9 的 S5 CRT tie-breaker 闭合。

### 0.9 2026-08-04 E3 S5 CRT tie-breaker 与 strict pass 台账

本轮目标：承接 0.8 的 CRT 持平边界，只做可写专利的 gold-free 修复，验证 MyAgent + Qwen3-32B 是否能在 WTQ/TabFact/CRT 三个 paired 多 seed 数据集上都严格超过 MACT。

机制变更：

1. 在 `MyAgent/code/my_agents.py` 的 `_canonicalize_crt_scalar` 中补 CRT scalar 输出规范化。
2. 规则 A：当问题问 `difference` 且模型给出负数标量时，输出非负差值。
3. 规则 B：当问题问 country/nation 且模型输出 IOC/国家代码时，展开为国家名。
4. 规则不读取 gold、不按样本 ID 分支、不改评估器；对应测试在 `tests/test_myagent_pipeline.py::test_crt_scalar_normalization_handles_difference_sign_and_country_codes`。
...
|---|---:|---|
| paired CRT diagnosis | 旧 CRT100：MyAgent `62/100`，MACT `62/100`；`mact_only=12`、`myagent_only=12`、`both_wrong=26` | `extract_crt_tiebreaker_diag.py`、`summary/crt_tiebreaker_diag.md`、`cases/*.jsonl` |
| canonicalizer replay | 旧 MyAgent CRT100 重放新 canonicalizer：`64/100` vs MACT `62/100`，无 correct->wrong | `simulate_crt_canonicalizer_patch.py`、`summary/s5_crt_canonicalizer_replay_summary.md` |
| affected-slice fresh | 25 条：新 MyAgent `16/25`、旧 MyAgent `12/25`、MACT `12/25`；failed/missing `0/0` | `prepare_crt_affected_slice.py`、`myagent_s5_affected_slice/`、`summary/s5_affected_slice_real_rerun_summary.md` |
| full CRT100 fresh | 新 MyAgent `65/100` vs MACT `62/100`；token ratio `0.7979`；failed/missing `0/0` | `prepare_crt_paired100_input.py`、`myagent_s5_crt_paired100_full_rerun/`、`eval/crt_qwen3-32b-local_eval.json` |
| S5 final combined | WTQ `76/100 > 74/100`，TabFact `91/100 > 87/100`，CRT `65/100 > 62/100`；overall `232/300 > 223/300`，token ratio `0.5662` | `summarize_s5_final.py`、`summary/e3_s5_final_combined_summary.json/md`、`s5_final_result.md` |

最终结论：

| dataset | MyAgent | MACT | delta | token ratio | strict |
|---|---:|---:|---:|---:|---|
| WTQ | 76/100 | 74/100 | +2 | 0.5762 | pass |
| TabFact | 91/100 | 87/100 | +4 | 0.2571 | pass |
| CRT | 65/100 | 62/100 | +3 | 0.7979 | pass |
| Overall | 232/300 | 223/300 | +9 | 0.5662 | pass |

当前可写入专利/专家材料的结论：在 Qwen3-32B 当前 paired 多 seed Gate-50 口径下，MyAgent 通过选择性风险协作、确定性审计和答案契约规范化，在 WTQ/TabFact/CRT 三个数据集上均严格超过 MACT，且总体 token 成本约为 MACT 的 `56.62%`。边界：CRT 单项 token 只降低到 MACT 的 `79.79%`，不是 `<=75%`；多模型外延仍需等新增模型/API 候选。

恢复/复核命令：

```bash
cd /home/ubuntu/lzz/MyAgent
/home/ubuntu/miniconda3/envs/lzz-agent/bin/python -m unittest discover -s tests -p 'test_myagent_pipeline.py' -k 'crt_'

cd /home/ubuntu/lzz/MACT
python outputs/server_runs/qwen3_32b_policy_v6c_e3_s5_crt_tiebreaker_diag_20260804_2225/summarize_s5_final.py
sed -n '1,120p' outputs/server_runs/qwen3_32b_policy_v6c_e3_s5_crt_tiebreaker_diag_20260804_2225/summary/e3_s5_final_combined_summary.md
```
...
|---|---|
| 总体准确率是否超过 MACT | 是。最新 paired 多 seed Gate-50/S5 合计 MyAgent `232/300`，MACT `223/300`，净胜 `+9`；历史 full200 阶段证据为 MyAgent `489/600` vs MACT `450/600` |
| token 是否仍明显低于 MACT | 是。最新 S5 overall token ratio 为 `0.5662`；分项为 WTQ `0.5762`、TabFact `0.2571`、CRT `0.7979`。CRT 单项仍低于 MACT 但节省幅度不如前两项，总体显著低于 MACT |
| 三个数据集是否都超过 MACT | 是。最新 S5 paired 多 seed 口径：WTQ `76/100 > 74/100`，TabFact `91/100 > 87/100`，CRT `65/100 > 62/100` |
| 当前项目是否可作为阶段证据 | 可以写成“Qwen3-32B 当前 paired 多 seed 三数据集单项均超过 MACT，且总体 token 显著更低”的阶段证据；正式论文/专家材料仍建议补细粒度消融和多模型 gate |
| 现在是否继续跑旧本地模型 | 不继续跑 no-go 模型；当前只围绕 Qwen3-32B + MyAgent 机制优化 |
| 下一步实验策略 | 不继续刷单数据集分数；先同步 S5 到正式实验包和专利草稿，再等待新增模型/API 候选按 Gate-10 / Gate-50 / Gate-150 / paired-200 扩展 |
...
| P4 | 新 seed 泛化验证 | completed through S5 2026-08-04: paired multi-seed strict all-dataset pass | P4b 是早期边界；E3/S4/S5 已完成 Seed-C/D paired 多 seed 验证，最终 S5 为 WTQ/TabFact/CRT 三项均超过 MACT，overall token ratio `0.5662` | MACT `qwen3_32b_policy_v6c_e3_s5_crt_tiebreaker_diag_20260804_2225/` + 本 PRD `0.9` |
| P5 | 多模型 gate | pending | 新模型/API 候选按 Gate-10 -> Gate-50 -> Gate-150 -> paired-200 漏斗筛选 | MACT `<model_tag>_gate*/` |
| P6 | 正式实验包 | updated through S5 2026-08-04 | 可给专家/专利代理人的实验包：方法说明、表格、消融、P4b after-targeted 正证据、E3/S5 strict pass、E4 no-candidate、多模型 gate 计划和复现实验命令 | MACT `qwen3_32b_patent_experiment_package_20260801_2155/` + MyAgent PRD |
...
| E2 | 机制消融补强 | `legacy`、`no_strong_verification`、`no_deterministic_shortcuts` 已有 coarse Gate-50；WTQ targeted fixes 已有 TDD + offline projection + fresh affected-slice `9/9` + after-targeted P4b `121/150` vs MACT `111/150`；专利机制证据矩阵已合并 full200/coarse/attribution；2026-08-04 新增 fine-grained mechanism audit，串联 S2 before/after fresh 与 S5 canonicalizer replay/fresh | 至少证明风险协作/劝返、确定性审计、证据保留中 2-3 个模块有贡献，并明确 offline 与 fresh 证据边界 | completed for current Qwen3 patent draft; standalone no-evidence-retention switch optional only if patent counsel requires narrower causality |
| E3 | 多 seed 稳定性 | 已从早期 current-only 边界推进到 S5 strict pass：S4 paired 为 overall pass 但 CRT tie；S5 CRT tie-breaker 后最终 WTQ `76/100 > 74/100`、TabFact `91/100 > 87/100`、CRT `65/100 > 62/100`、overall `232/300 > 223/300`、token ratio `0.5662`、overall failed/missing MyAgent `0/0`、MACT `4/4` | 证明 Qwen3-32B 当前 paired 多 seed 三数据集均超过 MACT，且总体 token 显著更低；仍需多模型外延证明跨模型稳定性 | completed 2026-08-04 S5 |
| E4 | 多模型 gate | 新本地模型或外部 API 模型按 Gate-10 -> Gate-50 -> Gate-150 -> paired-200 漏斗执行；2026-08-05 00:02 readiness audit 仍为 `no_candidate_wait`，未发现未测本地模型/API profile | 至少 1 个额外模型给出可解释结果；no-go 模型不扩 full200 | pending/no-candidate audited 2026-08-05 00:02 |
...
2. E3 已完成到 S5 strict pass：早期 current-only 与 S4 CRT tie 都保留为边界过程，最终 S5 通过最小 CRT gold-free canonicalizer 修复，完成 paired 多 seed 三数据集严格超过 MACT。
...
5. `latest_current_patent_experiment_section_zh.md` 是当前最新实验章节收口稿，已经纳入 full200、P4b after-targeted、机制矩阵、E3 边界、E3 S2 after-guard fresh 和 E4 no-candidate。
6. `next_validation_checklist_zh.md` 是服务器清理/扩容后的下一次验证清单，当前应优先等新模型/API 候选，而不是重跑已完成 WTQ fresh。

当前写作边界：E5/E6 可以支撑“Qwen3-32B full200 阶段原型有效”“P4b after-targeted new-seed 小样本三数据集均超过 MACT”“E3 S2 semantic guard 小样本机制门槛通过”和“机制设计清晰”；E3 仍不能替代 multi-seed 稳定性正证据，E4 只能写作 no-candidate readiness，仍不能替代多模型证据和最终正式实验表。
...
```text
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_fine_grained_mechanism_ablation_audit_20260804_2333/build_fine_grained_mechanism_ablation_audit.py
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_policy_v6c_e3_fine_grained_mechanism_ablation_audit_20260804_2333/summary/fine_grained_mechanism_ablation_audit.json
...
5. 专利机制证据矩阵将 full200 anchor `489/600` vs MACT `450/600`、`no_strong_verification` overall `-8/150`、`no_deterministic_shortcuts` overall `-15/150`、WTQ gain rows `24/25` strong-verification tag、TabFact gain rows `8/9` deterministic-audit tag 合并为可复核证据表；细粒度机制审计进一步确认 S2 before/after fresh 从 representative recovered `4/12` 到 `8/12`、no-harm `17/18` 到 `18/18`，S5 CRT canonicalization replay `64/100 > 62/100` 且 full CRT100 fresh `65/100 > 62/100`。当前机制消融对 Qwen3 专利草稿已够用；后续硬边界是 E4 多模型外延证据。
...
| E3 multi-seed 稳定性 | completed S5 strict pass | 原始 Seed-C/D current-only 合计 `212/300` 是边界基线；S2/S3/v6c boundary fresh/S4 paired/S5 CRT tie-breaker 已完成。S5 final 为 MyAgent `232/300` vs MACT `223/300`、token ratio `0.5662`、overall failed/missing MyAgent `0/0`、MACT `4/4`，WTQ/TabFact/CRT 三项均严格超过 MACT |
| 多模型 gate | pending/no-candidate audited 2026-08-05 00:02 | 最新 E4 readiness 为 `no_candidate_wait`，未发现未测本地模型或 API provider profile；新本地模型或 API key 出现后才按 Gate-10 -> Gate-50 -> Gate-150 执行，不重跑已 no-go 模型 |
...
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/build_current_completion_gap_audit.py
/home/ubuntu/lzz/MACT/outputs/server_runs/qwen3_32b_patent_experiment_package_20260801_2155/latest_completion_gap_audit_current_zh.md
...
当前在线状态：2026-08-05 00:02 CST 两个 Qwen3-32B endpoint 按用户要求保持常驻：GPU `2,3` -> `http://127.0.0.1:8000/v1`，GPU `0,1` -> `http://127.0.0.1:8001/v1`，served model 均为 `qwen3-32b-local`。除非快速切换模型或用户要求释放，不主动停止服务。WTQ fresh、P4b after-targeted、E3 Seed-C/D current-only、E3 离线边界诊断、E3 budget probe、semantic-boundary plan、S2 after-guard fresh、S3 after-guard current-only rerun、Seed-D WTQ/TabFact boundary diagnosis/projection/fresh、S4 paired MACT、S5 CRT tie-breaker 和低成本细粒度机制消融审计均已完成；00:02 E4 readiness 仍为 `no_candidate_wait`，下一次在线实验不再优先刷 Qwen3 单数据集，而是等待新增模型/API 后进入 E4 gate。

当前边界审计结论：Qwen3-32B 主锚点已达标，S2 after-guard fresh 已证明 P0/P1 guard 在 affected-slice/no-harm 小样本上通过，v6c boundary fresh 已把 Seed-C/D current-only 推到 paired MACT candidate；S4 paired MACT 通过 existing criteria 但 CRT 持平；S5 CRT tie-breaker 已把最终 paired 多 seed 结果推进为 WTQ/TabFact/CRT 三项均严格超过 MACT。后续不要把 S4 单独写成 strong strict，应引用 S5；多模型仍只能写 pending/no-candidate。

正式结果台账：2026-08-01 新增 `build_current_formal_result_ledger.py`，从 frozen full200 summary、P4b summary、WTQ fresh、E3 current summaries、E3 boundary diagnosis、正式模板和 latest preflight 生成 `latest_formal_result_ledger_current.json/md`。2026-08-04 23:03 已手工同步 S4/S5 结果，台账用于专家/专利表格填充，明确区分 `48` 条 completed rows 和 `1` 条 pending row，不把多模型 gate 写成已完成。

当前专利实验章节：`build_current_patent_experiment_section.py` 从 full200、P4b after-targeted、机制矩阵、E3 boundary diagnosis、E3 budget probe、semantic-boundary plan、S2 after-guard fresh、E4 readiness 和 formal ledger 生成 `latest_current_patent_experiment_section.json/md`。2026-08-04 23:03 已手工同步 S5 strict pass；该章节可用于专家/专利代理人理解当前证据，可以写 Qwen3 paired 多 seed 三数据集均超过 MACT，但不会把多模型写成已完成。

当前完成度审计：`build_current_completion_gap_audit.py` 从 current/latest evidence 生成 `latest_completion_gap_audit_current.json/md`。它替代旧 `completion_gap_audit_20260801_2244_zh.md` 的当前判断用途；旧文件只作历史记录，不能再作为当前缺口结论。

当前正式台账状态：`qwen3_strict_goal_complete_e4_pending`，原因是 Qwen3 strong strict 主目标已由 S5 闭合，但 E4 尚无可启动的新模型/API 候选，final closeout 仍需等 E4 candidate 或明确接受 no-candidate 边界。
...
最新一致性审计入口：MACT 专利实验包内 `latest_patent_package_consistency_audit.json/md`；当前目标状态为 `qwen3_strict_goal_complete_e4_pending`，因为 Qwen3 paired 多 seed strict 已完成，但多模型 gate 和最终 closeout 尚未完成。时间戳审计文件会随每次同步刷新，不再要求 PRD 逐个硬编码最新文件名。
