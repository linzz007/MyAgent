...
def render_progress_snapshot(myagent_root: Path, mact_root: Path, run_dir: Path) -> str:
    return "\n".join(
        [
            "#!/usr/bin/env bash",
            "set -euo pipefail",
            ...
            f"RUN_DIR={shell_quote(run_dir)}",
            'cd "$MYAGENT_ROOT"',
            "python scripts/server/report_run_progress.py \\",
            '  --run-dir "$RUN_DIR" \\',
...
def render_run_logged(myagent_root: Path, mact_root: Path, run_dir: Path) -> str:
    return "\n".join(
        [
            "#!/usr/bin/env bash",
            "set -euo pipefail",
            f"RUN_DIR={shell_quote(run_dir)}",
            'if [ "$#" -lt 2 ]; then',
            '  echo "usage: bash run_logged.sh <step-name> <command> [args...]" >&2',
            "  exit 2",
            "fi",
            'STEP_NAME="$1"',
            "shift",
            'mkdir -p "$RUN_DIR/logs/controller" "$RUN_DIR/summary"',
            'LOG_PATH="$RUN_DIR/logs/controller/${STEP_NAME}.nohup.log"',
            'PID_PATH="$RUN_DIR/logs/controller/${STEP_NAME}.pid"',
            'JOURNAL="$RUN_DIR/summary/run_journal.md"',
            "{",
            '  echo ""',
            '  echo "## $(date -Is) START ${STEP_NAME}"',
            '  printf -- "- command:"',
            '  printf " %q" "$@"',
            '  echo ""',
            f'  echo "- myagent_git_head: $(git -C {shell_quote(myagent_root)} rev-parse --short HEAD 2>/dev/null || true)"',
            f'  echo "- mact_git_head: $(git -C {shell_quote(mact_root)} rev-parse --short HEAD 2>/dev/null || true)"',
            '  echo "- log: ${LOG_PATH}"',
            '  echo "- pid_file: ${PID_PATH}"',
            "} >> \"$JOURNAL\"",
            "export RUN_DIR STEP_NAME LOG_PATH JOURNAL",
            "nohup bash -c '",
            "set +e",
            "\"$@\"",
            "STATUS=$?",
            "{",
            "  echo \"\"",
            "  echo \"## $(date -Is) END ${STEP_NAME}\"",
            "  echo \"- exit_status: ${STATUS}\"",
            "  echo \"- log: ${LOG_PATH}\"",
            "} >> \"$JOURNAL\"",
            "if [ -x \"$RUN_DIR/run_progress_snapshot.sh\" ]; then",
            "  bash \"$RUN_DIR/run_progress_snapshot.sh\" >/dev/null 2>&1 || true",
            "fi",
            "exit \"$STATUS\"",
            "' bash \"$@\" > \"$LOG_PATH\" 2>&1 < /dev/null &",
            'echo $! > "$PID_PATH"',
            'echo "[logged] ${STEP_NAME} pid=$(cat "$PID_PATH") log=${LOG_PATH}"',
            'echo "[logged] progress: bash $RUN_DIR/run_progress_snapshot.sh && tail -80 $RUN_DIR/summary/progress_snapshot.md"',
            "",
        ]
    )
...
12. `bash run_progress_snapshot.sh`

For long-running steps, prefer:
...
Use `run_logged.sh` for every long-running command. It writes:

- `logs/controller/<step-name>.nohup.log`: stdout/stderr from the command.
- `logs/controller/<step-name>.pid`: background process id.
- `summary/run_journal.md`: command, git heads, start time, finish time, and exit status.
- `summary/progress_snapshot.md` and `summary/progress_snapshot.json`: generated by `run_progress_snapshot.sh`.
...
tail -100 logs/controller/<step-name>.nohup.log
ps -p $(cat logs/controller/<step-name>.pid)
```

Send back `summary/progress_snapshot.md`, `summary/main_baseline_summary.md`, and the relevant controller log tail whenever a run stalls, exits, or produces unexpected row counts.
...
- `summary/progress_snapshot.md` and `summary/progress_snapshot.json`
- `summary/run_journal.md`
- `logs/controller/*.nohup.log` and `logs/controller/*.pid`
"""
...
def prepare(args: argparse.Namespace) -> Path:
    ...
    write_text(run_dir / "run_progress_snapshot.sh", render_progress_snapshot(myagent_root, mact_root, run_dir), executable=True)
    write_text(run_dir / "run_logged.sh", render_run_logged(myagent_root, mact_root, run_dir), executable=True)
